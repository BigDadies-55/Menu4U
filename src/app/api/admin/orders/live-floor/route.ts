...
export async function GET(req: Request) {
  ...
  const tables = Array.from(tableMap.entries()).map(([tableNumber, tOrders]) => {
    ...
    const hasLongPrep = allItems.some(
      i => i.itemStatus === "PREPARING" && i.firedAt && (now - i.firedAt.getTime()) / 60000 > 15
    );
    const isVeryOld = ageMin > 50;
    if (hasLongPrep || isVeryOld) status = "red";

    const mains = allItems.filter(i => i.course === 2);
    const allMainsDone = mains.length > 0 && mains.every(i => ["DONE", "DELIVERED"].includes(i.itemStatus));
    const oldestMainDone = mains.length > 0 ? Math.max(...mains.map(i => i.doneAt?.getTime() ?? 0)) : 0;
    if (status === "green" && allMainsDone && oldestMainDone && (now - oldestMainDone) / 60000 > 5) {
      status = "purple";
    }

    const statusTag =
      status === "red"    ? (hasLongPrep ? "⏱ עיכוב הכנה" : "⚠ שולחן ותיק")
      : status === "purple" ? "✦ הצע קינוח"
      : "✓ זרימה תקינה";
