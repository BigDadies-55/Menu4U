        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 700, background: "rgba(0,0,0,0.65)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div dir="rtl" style={{ background: "rgba(15,14,22,0.98)", backdropFilter: "blur(24px)", WebkitBackdropFilter: "blur(24px)", border: `1px solid ${G_BORDER_C}`, borderRadius: 20, width: "100%", maxWidth: 360, padding: 28, textAlign: "center", boxShadow: "0 20px 60px rgba(0,0,0,0.6)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <button onClick={() => signOut({ callbackUrl: "/login" })} style={{ padding: "10px", borderRadius: 12, border: `1px solid ${G_BORDER_C}`, background: "transparent", color: G_MUTED_C, fontWeight: 600, fontSize: 14, cursor: "pointer", fontFamily: "inherit" }}>כבר דיווחתי — צא</button>
              <button onClick={() => setLogoutPrompt(false)} style={{ padding: "10px", borderRadius: 12, border: `1px solid ${G_BORDER_C}`, background: "transparent", color: G_MUTED_C, fontWeight: 600, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>ביטול</button>
            </div>
          </div>
        </div>
      )}
